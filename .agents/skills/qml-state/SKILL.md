...
- If a GTK entrypoint is a symlink into `/usr/share/themes` or another system-owned location, do not write through it directly. Replace it with a user-owned file first so the apply pipeline does not abort on permission errors and skip later reload steps.
