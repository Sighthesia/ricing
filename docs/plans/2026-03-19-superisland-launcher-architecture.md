...
*   **Environmental Resonance (Highlight Ripple):** As the Launcher expands, a high-tech highlight ripple propagates outward from the center of the SuperIsland. **Crucially, this ripple is only visible on the shell's own surfaces (desktop background/wallpaper, panels, and bar components)**, without dimming, blocking, or drawing over regular tiled Niri application windows.
...
4.  **The Ripple Mask:** To achieve the environmental highlight ripple without rendering over app windows, we use a global desktop-level Layer Shell (Bottom/Background layer) that triggers an expanding radial gradient shader or opacity wave, synchronized with the `GrowthStage` animation. Panel windows use a matching visual ping.
